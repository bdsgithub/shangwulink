...
namespace HF.Clound.DingDingForm
{
    class Program
    {
        ...
        //其他项目打开的此exe应用程序，参数都在args里面
        static void Main(string[] args)
        {
            //传过来的值都在args里面
            HF.Cloud.BLL.Common.Logger.Error("ddForm进来了！");
            Console.WriteLine("ddForm进来了！");
            Console.WriteLine("参数个数：" + args.Length);
            HF.Cloud.BLL.Common.Logger.Error("ddForm进来了！" + args);
            Console.WriteLine("ddForm进来了！" + args);
            // HF.Cloud.BLL.Common.Logger.Error("第一个值：" + args[0]);
            //Console.WriteLine("第一个值："+args[0]);
            //Console.WriteLine("第二个值：" + args[1]);
            if (args.Length > 0)//传过来的值都在args里面，可以直接用的
            {
            ...
            //Console.Read();
            int port = 8124;//这里是端口，端口是自己设置的，只要跟自己本机不冲突的端口号都可以用
            ...
            {
               // HF.Cloud.BLL.Common.Logger.Error(e.ToString());
